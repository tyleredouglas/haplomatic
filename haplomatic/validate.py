...
def adaptive_windowing(
    ...
    extra_sizes = [coarse_sizes[-1] + 50000,
                   coarse_sizes[-1] + 100000]
    ...
    while start_pos < end_pos:
        ...
        for cs in coarse_sizes:
            ...
            if feats is None:
                continue
            meta = pd.DataFrame([feats])[cols]\
                     .replace([np.inf,-np.inf], np.nan)
            if meta.isna().any(axis=1).item():
                continue

            # initial‐flex on the smallest coarse window
            if not bumped and cs == coarse_sizes[0]:
                X_tab0 = torch.tensor(scaler.transform(meta),
                                     dtype=torch.float32)
                M0 = win[hap_cols].astype(float).values
                if len(M0) > max_snps:
                    M0 = M0[-max_snps:]
                else:
                    pad = np.zeros((max_snps-len(M0), M0.shape[1]),
                                   dtype=np.float32)
                    M0  = np.vstack([pad, M0])
                X_raw0 = torch.tensor(M0.astype(np.float32))\
                              .unsqueeze(0)
                with torch.no_grad():
                ...
                if pe0 < 0.35:
                    ...
                    logger.info(
                      f"  threshold bump: initial {cs}bp={pe0:.4f}<0.35 ⇒ new_thr={curr_thr:.4f}"
                    )

            X_tab = torch.tensor(scaler.transform(meta),
                                 dtype=torch.float32)
            M     = win[hap_cols].astype(float).values
            if len(M) > max_snps:
                M = M[-max_snps:]
            else:
                pad = np.zeros((max_snps-len(M), M.shape[1]),
                               dtype=np.float32)
                M   = np.vstack([pad, M])
            X_raw = torch.tensor(M.astype(np.float32))\
                          .unsqueeze(0)
            with torch.no_grad():
                pe = float(model(X_raw, X_tab).item())
            te = feats.get("error", np.nan)

            logger.info(f"  Window {start_pos}-{we} ({cs}bp): "
                        f"Pred={pe:.4f}, True={te:.4f}")

            if best is None or pe < best["pe"]:
                best = {"pe":pe, "we":we, "cs":cs,
                        "te":te, "feats":feats, "win":win}
            if pe <= curr_thr:
                logger.info(f"    coarse hit @ size {cs}bp: "
                            f"{pe:.4f} ≤ {curr_thr:.4f}")
                found = True
                break
        ...
        if not found and best is not None:
            ...
            if best["pe"] <= slack_thr:
                logger.info(
                  f"  no coarse ≤ {curr_thr:.4f}, "
                  f"but best {best['pe']:.4f} ≤ slack {slack_thr:.4f}; accepting best"
                )
                found = True
            else:
                logger.info(
                  f"  no coarse ≤ {curr_thr:.4f} or slack; scanning extra {extra_sizes}"
                )
                for cs in extra_sizes:
                    ...
                    te = feats.get("error", np.nan)
                    logger.info(
                      f"  extra window {cs}bp: Pred={pe:.4f}, True={te:.4f}"
                    )
                    if pe < best["pe"]:
                        best = {"pe":pe, "we":we, "cs":cs,
                                "te":te, "feats":feats, "win":win}
        ...
        ref_thr = err_thr + refine_flex

        logger.info(
          f"  refine between {low}–{cs0}bp in steps of {step_w}bp"
        )
        for W in range(low, cs0+1, step_w):
            ...
            if pe < best["pe"] or pe <= ref_thr:
                best = {"pe":pe, "we":we, "cs":W,
                        "te":te, "feats":feats, "win":win}
                if pe <= ref_thr:
                    logger.info(
                      f"      refine hit @ {W}bp: {pe:.4f} ≤ {ref_thr:.4f}"
                    )
                    break
        ...
        logger.info(
          f"Recording window @ {ws}-{we} ({wsz}bp): Pred={pe:.4f}, True={te:.4f}"
        )
...
def main():
    p = argparse.ArgumentParser(
        description="Adaptive windowing error‑predictor"
    )
    ...
    # build list of (chrom,start,end,subdf,sim,true_df) tasks
    def _gather_tasks():
        tasks = []
        for chrom, start, end in regions:
            sub = (
                snp_df
                .query("chrom==@chrom and pos>=@start and pos<=@end")
            ...
            for sim in sims:
                ...
                tasks.append((chrom, start, end, sub, sim, true_df))
        return tasks
    ...
    if args.workers > 1:
        logger.info(f"Launching ThreadPoolExecutor with {args.workers} workers")
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            fut_map = {
                ...
                for (chrom, start, end, sub, sim, true_df) in tasks
            }
            for fut in as_completed(fut_map):
                chrom, start, end, sim = fut_map[fut]
                out = fut.result()
                if not out.empty:
                    out["sim"]    = sim
