...
def compute_effective_rank(X: np.ndarray) -> float:
    ...
    s_norm = s / (s.sum() + 1e-8)
    ent = -(s_norm * np.log(s_norm + 1e-8)).sum()
...
class FeatureBuilder:
    ...
    multi-chain diagnostics (r̂, ESS) are omitted.
    """
    ...
    def _posterior_p(self, X: np.ndarray, b: np.ndarray):
        ...
        samp = mcmc.get_samples(group_by_chain=True)     # shape (1, draws, …)
        p_draw = np.asarray(samp["p"]).reshape(-1, k)
        sigma_draw = np.asarray(samp["sigma"]).reshape(-1)

        extra = mcmc.get_extra_fields(group_by_chain=True)
        divg = extra["diverging"]
        steps = extra["num_steps"]
        div_rate = float(divg.mean())
        max_depth = int(np.log2(steps + 1e-8).astype(int).max())
    ...
    def build(self,
        ...
        for (pop, (start, end)), info in windows.items():
            ...
            for i, col in enumerate(founder_cols):
                rec[f"lsei_{col}"] = float(lsei[i])
                rec[f"avg_SNPfreq_{col}"] = float(avgf[i])
...
def main() -> None:
    ...
    ap.add_argument("--snp-freqs", required=True,
                    help="CSV containing chrom,pos,<founders>,<pop1>,<pop2> columns")
